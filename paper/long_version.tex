...
\begin{table}
	...
	\begin{tabular}{l c c c c c}
		...
		\midrule 
		Trivial & 2.5e-06 & 2.25e-06 & 2.27e-06 & 2.23e-06 & 2.16e-06 \\ 
		KDE & 0.086 & 0.079 & 0.074 & 0.074 & 0.122 \\ 
		DS Correlated & 0.188 & 1.419 & 3.460 & 16.66 & 159.9 \\ 
		DS Correlated w/Config & 0.172 & 0.627 & 1.393 & 342.7 &  \\ 
		DS Independent & 0.044 & 0.050 & 0.103 & 0.268 & 2.608 \\ 
		DS Independent w/Config &  &  &  &  &  \\
		\midrule
	\end{tabular}
\end{table}
...
Based on the results from this experiment, it is clear that some methods are much faster than others. Fast generation of synthetic data is important in a self-service setting where researchers can generate their own data as needed. While KDE is a simple method, it quickly generates data that looks similar to the input data. DataSynthesizer in "Independent" mode is quick for a range of dataset sizes, but without configuration, it doesn't generate results that closely match the input dataset; e.g. the "Code" column should contain valid International Statistical Classification of Diseases and Related Health Problems, Tenth Revision (ICD-10) codes, but instead has random strings that are similar in length to actual ICD-10 codes.

In all cases, synthetic data generation is a CPU intensive task; no parallization has been implemented by either KFP nor the plugins used. While memory usage does increase as data size increases, at the 100000 rows seen here, memory usage peaked at around 2GB for the Python process.
